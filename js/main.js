    ...
    for (var i = 0; i < a.length; i++) {
      ...
      if (now == curDate) {
        if (
          ...
          console.error(
            "проверь наличие судей в общем списке:" + checkJudge
          );
        }
      }
    }
    for (var m = 1; m < zal.length; m++) {
      for (var j = 1; j < zal[m]; j++) {
        $(".vertical-" + m).append(
          '<section><div class="date-' +
            m +
            '"></div><div class="time-' +
            m +
            '"></div><div class="case_number-' +
            m +
            '"></div><div class="sides-' +
            m +
            '"></div><div class="judges-' +
            m +
            '"></div><div class="essence-' +
            m +
            '"></div><div><select><option>розглядається</option><option>затримується +5хв</option><option>затримується +15хв</option><option>затримується +30хв</option></select></div></section>'
        );
